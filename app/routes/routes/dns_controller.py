...
@dns.get(path="/run_verification_dns", response_description="Get Succesfully")
def DNS_verification(db_connection: Session = Depends(get_db)):
    dn_service.run = threading.Thread(
        target=dn_service.run_verification, args=(db_connection,)
    )
...
@dns.patch(
    path="/update_dns",
    response_description="Update Succesfully",
    response_model=ResponseDns,
)
def update_dns(index: int, name_DNS: str, db_connection: Session = Depends(get_db)):
    return CrudDns.update_dns(db_connection, name_DNS, index)
